...
module.exports = {
  ...
  register: function(req, res) {
    ...
    return passport.authenticate('localRegister', function(err, user, challenge) {
      if (err) {
        // @TODO: log security incident
        return makeSessionPayload(req).then(function(payload) {
          return res.apiBadRequest('Registration failed', payload, { $error: err });
        });
      }

      else if (!user) {
        // @TODO: log security incident, include challenge
        var message = challenge ? challenge.message : 'Registration failed';
        return makeSessionPayload(req).then(function(payload) {
          return res.apiBadRequest(message, payload);
        });
      }

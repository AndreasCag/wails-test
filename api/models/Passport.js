...
module.exports = {
  ...
  /**
   * Register a user.
   * Returns error if registration has failed.
   */
  registerLocal: function (username, password, done) {
    User.create({
      'username': username
      ...
      var newPassport = {user: user.id, password: password, strategy: 'local'};

      sails.models.passport.create(newPassport)
        .then(passport => {
        ...
        .catch(err1 => {
          ...
          User.destroy(user).exec(function (err2) {
            if (err2) {
              return done(err2);
            }
            done(err1);
  ...
  verifyForRegister: function (username, password, next) {
    // @TODO: log security event
